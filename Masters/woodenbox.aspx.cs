...
public partial class Masters_woodenbox : System.Web.UI.Page
{
   ...
   public void bindreport()
    {
        ReportGrid.DataSource = db.Displaygrid("select distinct 0 as # , itemname as TemplateName ,0 as title from C");
        ReportGrid.DataBind();
    }
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
                ...
                for (int i = 0; i < grdwoodenbox.Rows.Count; i++)
                {
                    try
                    {
                        ...
                        db.insert("Insert Into nonstwood values('" + "0"+ "' ,'" + itemname + "' ,'" + txtwoood + "' )");
                bindreport();
            }
                    catch (Exception ex)
                    {
                ...
                case ("Select"):
                    {
                        ...
                        grdwoodenbox.DataSource = db.Displaygrid("select   itemname  , WoodenBoxSize  from nonstwood where iterlockid='" + 0+ "'");

                        grdwoodenbox.DataBind();

                        ViewState["CurrentTablewood"] = db.Displaygrid("select id as RowNumber,  itemname  , WoodenBoxSize from nonstwood where iterlockid='" + 0 + "'");
                        DataTable dtwood = new DataTable();
                        dtwood = db.Displaygrid("select *  from nonstwood where iterlockid='" + ViewState["EditID"] + "'");
