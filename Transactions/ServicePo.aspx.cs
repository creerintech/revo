...
public partial class Transactions_ServicePo : System.Web.UI.Page
{
    database db = new database();
    #region [private variables]

    servicepo Obj_PurchaseOrder = new servicepo();
                            ...
                            if (MaxId > 0)
                            {
                                if (ViewState["CurrentTable"] != null)
                                {
                                    for (int k = 0; k < PurOrderGrid.Rows.Count; k++)
                                    {
                                        ...
                                        {
                                            servicepo oDMPurchase_Order = new servicepo();
