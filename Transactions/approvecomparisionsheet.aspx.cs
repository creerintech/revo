...
public partial class Transactions_approvecomparisionsheet : System.Web.UI.Page
{ 
...
protected void BtnSave_Click(object sender, EventArgs e)
{
    ...
    for (int j = 0; j < GrdReq.Rows.Count; j++)
    {
        if ((GrdReq.Rows[j].Cells[6].Text) == "Approved")
        {
            ...
            db.insert("update Conversionpo set  status='" + "Approved" + "'  where enquiry='" + STR + "' ");
        }
        if ((GrdReq.Rows[j].Cells[6].Text) == "Authorised")
        {
            string STR = ((Label)GrdReq.Rows[j].FindControl("LblEntryId")).Text;
            db.insert("update Conversionpo set  status='" + "Authorised" + "'  where enquiry='" + STR + "' ");
        }
    }
    BindReportGrid("");
    GETHIGHLITE();
